...
<template>
  <div class="datePicker w-full" data-v-inspector="node_modules/vue3-persian-datepicker/index.vue:5:3">
    <input
      type="text"
      :name="name"
...
<thead>
  <tr>
    <th class="datePicker__th__button">
      <button type="button" @click="prevMonthHandle">ماه قبل</button>
    </th>
    <th class="datePicker__th__button">
    ...
    <th>
      <button type="button" @click="openYearList">{{ state.year }}</button>
    </th>
    <th class="datePicker__th__button">
  ...
  <tr>
    <th v-for="(item, index) in state.weekdaysMin" :key="index">{{ item }}</th>
  </tr>
</thead>
<tbody>
<tr v-for="(item, index) in state.calcNumberWeeks(
      state.offsetDay(state.firstDayMonth(state.year, state.month)),
      state.lastOffsetDay(state.lastDayMonth(state.year, state.month, state.daysInMonth(state.year, state.month))),
  ...
  <td v-for="(day, idx) in state.tableDays(
      ...
      :class="{
        ...
        'datePicker--active': !state.selected && state.today === day && state.checkYearMonth(state.year, state.month),
        'datePicker--highlight': state.today === day && state.checkYearMonth(state.year, state.month),
      }"
...
export default {
  ...
  setup(props, context) {
    ...
    state.date = context.attrs.modelValue; // Keep this or set a default format, e.g. "YYYY/MM/DD"
  } else {
    state.date = ''; // Set to an empty string or a default value instead of today's date
  }
    });
